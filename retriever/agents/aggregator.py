...
from models import (
    TaskQueue, AggregatedContent, StructuredSection, BlockRef,
    SIGNAL_RE_SEARCH,
)
...
def aggregate(task_queue: TaskQueue, results: list[dict]) -> dict:
    ...
    # Aggregator LLM이 실제 사용한 결과 인덱스 (1-based)만 출처로 수집
    # LLM이 반환하지 않으면 모든 결과를 폴백으로 사용
    used_raw = data.get("used_result_indices")
    if used_raw and isinstance(used_raw, list):
        used_indices: set[int] = {int(i) for i in used_raw if isinstance(i, (int, float))}
    else:
        used_indices = set(range(1, len(results) + 1))  # 폴백: 전체
    ...
    for i, r in enumerate(results, 1):
        ...
        if source_type == "web":
            url = r.get("source", {}).get("url", "") or r.get("id", "")
            if url and url not in seen_urls:
                seen_urls.add(url)
                web_refs.append(url)
        else:
            ref = r.get("block_ref")
            if ref and isinstance(ref, BlockRef) and ref.section_id not in seen_sections:
    ...
    logger.info("[DONE] Aggregator: %d개 섹션, 문서 출처 %d개, 웹 출처 %d개 (사용 결과 %d/%d)",
                len(sections), len(block_refs), len(web_refs), len(used_indices), len(results))
